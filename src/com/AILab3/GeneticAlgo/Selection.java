...
public class Selection
{
                                  ...
                                  String selectionMethod, boolean aging)
    {
        ...
        {
            // Aging
            selection_size = Constants.GA_POPSIZE - 1;
            population.sort(AlgoGene.BY_AGE);
            while (population.get(selection_size).age > 3) selection_size--;
        } else
            // Regular elitism
            selection_size = (int) (Constants.GA_POPSIZE * Constants.GA_ELITRATE);
        ...
        {
            case "sus":
                stochasticUniversalSampling(population, parents, Constants.GA_POPSIZE - selection_size, aging);
                break;
            case "tournament":
                tournamentSelection(population, parents, Constants.GA_POPSIZE - selection_size);
                break;
            default:
                randomSampling(population, parents, Constants.GA_POPSIZE - selection_size, aging);
        }
                                                    ...
                                                    int amountOfChildrenToGenerate, boolean aging)
    {
        ...
        int[] aggregateInvertFitness = new int[Constants.GA_POPSIZE];
        int first = 0;
        if (aging)
        ...
        for (int i = first + 1; i < Constants.GA_POPSIZE; i++)
        ...
        int rouletteSize = aging ? amountOfChildrenToGenerate * 2 : Constants.GA_POPSIZE / 4;
        int fitnessJump = (aggregateInvertFitness[Constants.GA_POPSIZE - 1] / rouletteSize);
        // start randomly between 0 and the jump distance
        int start = Constants.r.nextInt(fitnessJump);
    ...
    {
        parents.clear();
        int i1, i2, f = 0, l = Constants.GA_POPSIZE - 1, m = Constants.GA_POPSIZE;
        if (aging)
        {
        ...
        for (int i = childrenToGenerate; i < Constants.GA_POPSIZE; i++)
        {
            do
            {
                i1 = Constants.r.nextInt(m) + f;
                i2 = Constants.r.nextInt(m) + f;
            } while (i1 == i2);
    ...
    private static void tournamentSelection (Vector<AlgoGene> pop, Vector<AlgoGene> parents, int childrenToGenerate)
    {
        ...
        int _w = pop.get(Constants.GA_POPSIZE - 1).fitness;
        int _mf1, _mf2, i1, i2;
        for (int i = 0; i < childrenToGenerate; i++)
        {
            ...
            for (int j = 0; j < sampleSize; j++)
            {
                ...
                tournament.add(pop.get(Constants.r.nextInt(Constants.GA_POPSIZE / 2)));
